...
export class OverlayService {
  ...
  showConfirmDialog(options: NgConfirmDialogOptions): Promise<boolean | null> {
    ...
    const confirmation: Confirmation = {
      ...
      rejectButtonStyleClass: `${options.rejectButtonStyleClass} ${options.buttonFull ?? 'w-100'} p-button-${options.rejectColor || 'danger'} p-button-${options.rejectAppearance || 'outlined'} p-button-${options.buttonSize}`,
    }
    instance.style = options.style;
    ...
    instance.rejectLabel = 'خیر';
    instance.acceptLabel = 'بله';
    instance.autoZIndex = options.autoZIndex != undefined ? options.autoZIndex : true;
    instance.breakpoints = options.breakpoints;
